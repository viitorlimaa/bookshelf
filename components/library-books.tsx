...
export function LibraryBooks({ searchParams }: LibraryBooksProps) {
  const { books, setBooks, removeBook } = useLibrary();
  ...
  useEffect(() => {
    // Se já tiver livros no contexto, não precisa buscar
    if (books.length > 0) {
      setFetched(true);
      return;
    }
    ...
    async function fetchBooks() {
      ...
      try {
        ...
        const normalized: Book[] = data.map((b: any) => ({
          ...b,
          genres: Array.isArray(b.genres) ? b.genres.map((g: any) => g.name) : [],
  ...
  }, [setBooks]); // só depende do setter do contexto
  ...
  return (
    <LibraryToaster>
      ...
      {!loading && !error && filteredBooks.length > 0 && (
        <BookGrid books={filteredBooks} onDelete={removeBook} />
      )}
    </LibraryToaster>
  );
}
