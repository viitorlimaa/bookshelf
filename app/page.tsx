...
import { BookOpen, BookMarked, CheckCircle, FileText, Plus, TrendingUp } from "lucide-react"
...
export default function Dashboard() {
  ...
  const totalPagesRead = finishedBooks.reduce((total, book) => total + (book.pages || 0), 0)
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-foreground text-balance">Bem-vindo à sua Biblioteca Pessoal</h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          <StatsCard title="Total de Livros" value={totalBooks} description="Em sua biblioteca" icon={BookOpen} />
          <StatsCard
              ...
              {currentlyReading.length > 0 ? (
                <div className="space-y-4">
                  {currentlyReading.map((book) => {
                    const progress =
                      book.pages && book.currentPage ? Math.round((book.currentPage / book.pages) * 100) : 0

                    return (
                      <div key={book.id} className="flex items-center space-x-4 p-4 bg-muted rounded-lg">
                        <div className="relative h-16 w-12 flex-shrink-0">
                          <Image
                            src={book.cover || "/placeholder.svg?height=64&width=48&query=book%20cover"}
                            alt={book.title}
                            fill
                ...
                {recentBooks.map((book) => (
                  <div key={book.id} className="flex items-center space-x-3">
                    <div className="relative h-12 w-9 flex-shrink-0">
                      <Image
                        src={book.cover || "/placeholder.svg?height=48&width=36&query=book%20cover"}
                        alt={book.title}
                        fill
                        className="object-cover rounded"
                      />
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-muted-foreground">
                        Atualizado {new Date(book.updatedAt).toLocaleDateString("pt-BR")}
                      </p>
                    </div>
                  </div>
                ))}
